...
export async function updateOrderFiles(orderId, files) {
  const { data, error } = await supabase.from('orders').update({
    files,
    updated_at: new Date().toISOString(),
  }).eq('order_id', orderId).select().single();
...
export async function getUploadHistory(email, limit = 12) {
  const { data } = await supabase
    ...
    .eq('status', 'active')
    // Records from before the Uploadcare switch point at dead storage —
    // offering them for re-use puts unfetchable files on fresh orders.
    .ilike('secure_url', '%ucarecdn.com%')
